...
namespace CMCSPart2.Controllers
{
    public class ManagerController : Controller
    {
        ...
        public ManagerController(InMemoryStore store) => _store = store;

        private string RequireRole()
        {
            var role = HttpContext.Session.GetString("Role");
            if (!string.Equals(role, "Manager", StringComparison.OrdinalIgnoreCase))
                throw new InvalidOperationException("Not a manager");
            return HttpContext.Session.GetString("Username") ?? "Manager";
        }
        ...
        {
            RequireRole();
            await _store.UpdateClaimStatusAsync(id, approval.Decision, "Manager", approval.Comments ?? "");
